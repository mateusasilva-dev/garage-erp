    ...
    render() {
      ...
      <style>
        ...
        * { box-sizing: border-box; font-family: "Inter", sans-serif; }
        ...
        .mobile-header .brand-text { color: #1f2937; display: flex; align-items: center; font-size: 20px; font-weight: 600; line-height: 30px; letter-spacing: normal; margin: 0; margin-top: -2px; }
        ...
        .brand-logo { width: 56px; height: 56px; border-radius: 12px; display: grid; place-items: center; background: #ffffff; border: 1.25px solid var(--sidebar-border); box-shadow: 0px 4px 6px 0px rgba(0,0,0,0.1), 0px 2px 4px 0px rgba(0,0,0,0.1); }
        .brand-logo img { width: 34px; height: 34px; filter: none; object-fit: contain; }
        
        .brand-text { display: flex; flex-direction: column; gap: 0px; }
        .brand-text strong { color: var(--text-primary); font-size: 18px; font-weight: 700; line-height: 28px; margin-top: -2px; }
        .brand-text span { color: var(--text-muted); font-size: 12px; font-weight: 400; line-height: 16px; }

        .menu-btn, .close-menu { 
          width: 40px; height: 40px; border: 0; border-radius: 8px; background: transparent; 
        ...
        .role-card {
          margin: 16px; min-height: 66px; padding: 13px; border: 1px solid var(--profile-border);
          border-radius: 8px; background: var(--profile-bg); display: flex; align-items: center; gap: 12px;
        }
        .role-card svg { width: 24px; height: 24px; }
        .role-card strong { display: block; color: var(--profile-color); font-size: 16px; font-weight: 600; line-height: 24px; text-transform: capitalize; }
        .role-card span { color: var(--text-muted); font-size: 12px; font-weight: 400; line-height: 16px; }

        .sidebar-nav { padding: 0 16px; flex: 1; overflow-y: auto;}
        .sidebar-nav ul { margin: 0; padding: 0; list-style: none; display: flex; flex-direction: column; gap: 8px; }
        
        .nav-link {
          min-height: 60px; border: 1px solid transparent; border-radius: 10px; padding: 0 16px;
          text-decoration: none; color: var(--text-soft); font-size: 18px; font-weight: 400; line-height: 24px;
          display: flex; align-items: center; gap: 16px; cursor: pointer; transition: background 0.2s;
        }
        .nav-link:hover { background: rgba(255,255,255,0.05); }
        .nav-link svg { width: 28px; height: 28px; flex-shrink: 0; }
        .nav-link.active, .nav-link[aria-current="page"] {
          color: #ffffff; border-color: var(--active-link-border); background: var(--active-link-bg); font-weight: 500;
        }

        .sidebar-footer {
          border-top: 1px solid var(--sidebar-border); padding: 16px; display: flex; align-items: center;
          justify-content: space-between; gap: 12px; min-height: 56px;
        }

        .user-info { min-height: 40px; display: flex; align-items: center; gap: 12px; flex: 1; }
        .avatar {
          width: 40px; height: 40px; border: 1px solid var(--avatar-border); border-radius: 999px;
          background: var(--avatar-bg); color: var(--text-primary); font-size: 18px; font-weight: 600;
          display: grid; place-items: center; text-transform: uppercase;
        }
        .user-info strong { display: block; color: var(--text-primary); font-size: 16px; font-weight: 500; line-height: 24px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        .user-info span { color: var(--text-muted); font-size: 12px; font-weight: 400; line-height: 16px; text-transform: capitalize; }
        .user-info > div { min-width: 0; }

        .logout { width: 20px; height: 20px; border: 0; border-radius: 0; background: transparent; color: var(--text-soft); display: flex; align-items: center; justify-content: center; padding: 0; cursor: pointer; font-size: 0; overflow: hidden; }
        .logout svg { width: 20px; height: 20px; }
        .logout-label { display: none; }

        .overlay {
          display: none;
          position: fixed; inset: 0; background: rgba(0, 0, 0, 0.5); z-index: 30; opacity: 0; transition: opacity 0.3s ease;
        }

        @media (max-width: 760px) {
          :host { height: auto; position: static; display: block; }
          ...
          .close-menu { display: flex; color: var(--text-primary); }
          .logout { width: 100%; min-height: 48px; border: 1px solid var(--sidebar-border); border-radius: 8px; background: rgba(249, 250, 251, 0.03); padding: 0 16px; gap: 8px; font-size: 16px; font-weight: 600; overflow: visible; margin-top: 16px;}
          .logout-label { display: inline; font-weight: 500; color: var(--text-soft); }
            ...
            ${navLinks
                .map((link) => {
                    ...
                    const isActive =
                        ...
                        (currentPath.endsWith("/") && link.url === "dashboard")
                            ? "active"
                            : "";
                    return `
              <li>
